...
class FeedPagination(PageNumberPagination):
    ...
    max_page_size = 50  # Optional: Limit max results per page

# -------------------- LOGGER --------------------------
logger = LoggerSingleton().get_logger()

# ------------------- CACHING CONFIGURATION ----------------------
CACHE_TIMEOUT = 1  # Cache timeout in seconds (1 second)
...
class PostListCreate(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if user.is_staff:  # Admin user
            # Admin can see all posts (public and private)
            return Post.objects.all().order_by('-created_at')
        else:
            # Normal users: only public posts or own private posts
...
class CommentRetrieveUpdateDestroy(generics.RetrieveUpdateDestroyAPIView):
    ...
    serializer_class = CommentSerializer
    # Only owner or admin can edit/delete; others can only read
...
class LikePostView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Invalidate feed cache for current user
        UserFeedView().clear_all_feed_page_caches()
